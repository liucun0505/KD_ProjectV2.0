...
void CDlgConnection::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_COMBO1, netComm);
	DDX_Control(pDX, IDC_EDIT1, edit_netadd);
	DDX_Control(pDX, IDC_EDIT2, edit_netpassword);
}
...
void CDlgConnection::OnBnClickedOk()
{
	...
	else if(m_NWGW.GetCurSel()==1)
	{
		pMain->NW_OR_GW=1;
		pMain->m_PeiZhi=".\\配置文件\\13762树控件NW.ini";
		pMain->m_PeiZhi_db=".\\配置文件\\13762DataNW.accdb";
	}
	if(netComm.GetCurSel()==0)
	{
		pMain->connectType=0;
	}
	else if(netComm.GetCurSel()==1)
	{
		pMain->connectType=1;
	}
	...
	CString strBaud , strDatabit;//,strNetIp,strNetPassword;
	...
	edit_netadd.GetWindowText(m_strNetAdd);
	edit_netpassword.GetWindowText(m_strNetPassword);

	CString strlistData = m_strCommPort + _T("☆") + strBaud + _T("☆") + m_strCommParity + _T("☆")+ m_strCommStopbit + _T("☆") +strDatabit+ _T("☆") + m_strMainMAC + _T("☆")+ m_strNetAdd + _T("☆")+ m_strNetPassword + _T("☆");
...
BOOL CDlgConnection::OnInitDialog()
{
	...
	try
	{
		...
		if(m_access.SelectDataFromTable(pMain->m_PeiZhi_db,_T("Config"),_T("MessageData"),stWhere,&strData,nHang))
		{
			...
			edit_netadd.SetWindowTextW(strlist[6]);
		}
	}
	catch (_com_error& e)
	{
...
void CDlgConnection::AddPort()
{
	...
	//m_combBaud.ShowWindow(false);
	netComm.AddString(_T("串口"));
	netComm.AddString(_T("网络"));
	netComm.SetCurSel(0);

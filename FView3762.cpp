...
void CFView3762::OnInitialUpdate()
{
	...
	m_editAddress.SetWindowTextW(_T("010000000000"));
...
void CFView3762::OnDblclkTreeData(NMHDR *pNMHDR, LRESULT *pResult)
{
	...
	else if(strZuming == "F0H扩展命令" && strWhere == "F032")
	{
		TYPESTALOG=1;
...
void CFView3762::SetUpdataListData()
{
	CMainFrame* pMain= (CMainFrame*)AfxGetApp()->GetMainWnd();
	int nHang= 0;
	CString strlist[10];
...
INT8U CFView3762::SendUpdataData(CString strName , int nChoose)
{
	CMainFrame * pMain = (CMainFrame*)AfxGetApp()->GetMainWnd();
	...
	INT8U Sendbufdata[2000] , AddrBuf[7];
...
SENDCONTINUE:
		...
		{
		case  0:
			//HostSendRcv376_2BufNode
			ack = gSimJzq.HostSendRcv376_2Buf(0x15 , F1 , Sendbufdata , Sendbuflen , m_ptSendQGDW376_2 , m_ptRecvQGDW376_2);
			break;
		case  1:
			m_gw13762.Code376_2_F0F008Data(0x43 , F8DataHead ,0x15 ,F1 ,Sendbufdata ,Sendbuflen ,ptSendQGDW376_2);
			m_tools.Buf_To_LenBuf(ptSendQGDW376_2.s_SendDataBuf , ptSendQGDW376_2.s_SendDataLen , Sendbufdata , Sendbuflen );
			//		//lc南网扩展标记
			ack = gSimJzq.HostSendRcv376_2BufNode(AddrBuf , 0xF0 , F8 , Sendbufdata , Sendbuflen , m_ptSendQGDW376_2 , m_ptRecvQGDW376_2);
			//m_tools._str16tobuf16( pMain->m_strMAddress , AddrBuf , temp16 , true);
...
void CFView3762::SaveDebugData(INT32U pos, INT8U *recvBuf, INT16U recvLen)
{
	...
	fseek(m_fp, pos, SEEK_SET);
            ...
            *(ch+i) = *(recvBuf+i);
			S=ch;            
			CStringArray* result = DivString(S);
			int nSize = result->GetSize();
			CString cstr;
			for (int i = 0; i < nSize; i++)
			{
			    cstr+=result->GetAt(i) + _T("\r\n");
			}
...
INT8U CFView3762::RecvDebugData(CString strName , INT8U *F8DataHead, int nChoose)
{
	...
	CString strShuxingBiaoshiLen;
	INT8U Recvbufdata[2000] , AddrBuf[7];
		...
		case 0:
			...
			else
			{
				if (m_ptRecvQGDW376_2.s_head.s_AFN == 0xF0 && m_ptRecvQGDW376_2.s_head.s_FN == F21)
				{
					...
					{
						if(m_ptRecvQGDW376_2.s_RcvDataLen < 300)
						{
							SetWindowText(_T("接收错误，停止\r\n"));
							SetWindowText(_T("-------------------------------------------------------------------------------\r\n"));
							return 0;
						}
						if(m_ptRecvQGDW376_2.s_RcvDataBuf[15]==0xFF&&m_ptRecvQGDW376_2.s_RcvDataBuf[m_ptRecvQGDW376_2.s_RcvDataLen-2]==0xFF){
							SetWindowText(_T("--------------------------------------------------------------------------------\r\n"));
							return 0;
						}
...
INT8U CFView3762::RecvDebugData_7(CString strName , INT8U *F8DataHead, INT8U Data_7,  int nChoose)
{
	CMainFrame * pMain = (CMainFrame*)AfxGetApp()->GetMainWnd();
	CString str = _T("======================================================\r\n");
	if (pMain->m_nChooseView == OPEN13762)
...
//			ack = gSimJzq.HostSendRcv376_2F0F8(AddrBuf ,0xF0 , F21 , Sendbufdata , Sendbuflen , m_ptSendQGDW376_2,ptRecvQGDW376_2 , m_ptRecvQGDW376_2);
		...
		case 0:
			if (m_ptRecvQGDW376_2.s_head.s_AFN == 0x00 && m_ptRecvQGDW376_2.s_head.s_FN == F2)
			{
				...
				CString strSn;
				strSn.Format(_T(", sn %d"),nReadStart);

				str = str + strSn + _T("收到否认帧\r\n");
				SetWindowText(str);
...
/*				if(resendcnt > 5)
			...
			else
			{
				...
				{
					CString strClear;
					m_editShow.GetWindowTextW(strClear);
					if(strClear.GetLength()>12000){
						m_editShow.SetWindowTextW(_T(""));
					}
					if(m_ptRecvQGDW376_2.s_RcvDataLen > 10)
					{
						
						SaveExtendStaVersion(nReadStart-1, &m_ptRecvQGDW376_2.s_RcvDataBuf[0], m_ptRecvQGDW376_2.s_RcvDataLen);

						SetWindowText(_T(">>>>接收成功:") + m_tools._buf16tostr16(&m_ptRecvQGDW376_2.s_RcvDataBuf[3] , 6 , false , false) + _T("\r\n"));
						//nReadStart++;
						resendcnt = 0;
					}
					else
...
void CFView3762::SaveExtendStaLog(INT32U sn, INT8U *recvBuf, INT16U recvLen)
{
	CMainFrame* pMain= (CMainFrame*)AfxGetApp()->GetMainWnd();
	int nHang= 0;
	char szANSIString[5000];
	CString strlist[25];
	CString strUpDataMessage,strUpDataMessageHead;

	CString stWhere = _T("ConfigName='STA的故障信息'");
	m_accessData.SelectDataFromTable(pMain->m_PeiZhi_db,_T("Config"),_T("MessageData"),stWhere,&strUpDataMessage,nHang);//.\\配置文件\\13762Data.accdb
	CString stWhereHead = _T("ConfigName='STA的故障信息表头'");
	m_accessData.SelectDataFromTable(pMain->m_PeiZhi_db,_T("Config"),_T("MessageData"),stWhereHead,&strUpDataMessageHead,nHang);//.\\配置文件\\13762Data.accdb
	//m_accessData.SelectDataFromTable(_T(".\\配置文件\\13762Data.accdb"),_T("Config"),_T("MessageData"),stWhere,&strUpDataMessage,nHang);
	m_tools.Split(strUpDataMessage , _T("☆") , strlist , nHang);
	...
	if  ( m_fp == NULL )
	{
		...
		if(m_fp != NULL) 
		{
			CString strTopLine = strUpDataMessageHead;
			WideCharToMultiByte(CP_ACP,WC_COMPOSITECHECK,strTopLine,-1,szANSIString,sizeof(szANSIString),NULL,NULL);
			fwrite(szANSIString,sizeof(szANSIString)/13-4,1,m_fp);
			//fwrite(szANSIString,sizeof(szANSIString)/13-9,1,m_fp);
		}
	}
	else
	{
	...
	if(m_fp != NULL) 
	{
		...
		CString strlistData[30];

		CString strSn;
		sn=sn+1;
		strSn.Format(_T("%d"),sn);

		strlistData[0] = strSn;

		//strlistData[1] = m_tools._strDaoxu(strAllTPdata.Mid(0, 12));
		strlistData[1] = m_tools._str16tostr10(m_tools._strDaoxu(strAllTPdata.Mid(0, 8)));//m_tools._strDaoxu(strMAC);//
		//strlistData[3] = m_tools._str16tostr10(m_tools._strDaoxu(strAllTPdata.Mid(20, 8)));//m_tools._strDaoxu(strAllTPdata.Mid(8 , 8));//
		strlistData[2] = m_tools._str16tostr10(m_tools._strDaoxu(strAllTPdata.Mid(8, 8)));//m_tools._str16tostr10(strAllTPdata.Mid(16 , 8));
		strlistData[3] = m_tools._str16tostr10(m_tools._strDaoxu(strAllTPdata.Mid(16, 8)));//m_tools._str16tostr10(strAllTPdata.Mid(24 , 8));

		strlistData[4] = m_tools._str16tostr10(m_tools._strDaoxu(strAllTPdata.Mid(24, 8)));//m_tools._str16tostr10(strAllTPdata.Mid(32 , 8));
		strlistData[5] = m_tools._str16tostr10(m_tools._strDaoxu(strAllTPdata.Mid(32, 8)));//m_tools._str16tostr10(strAllTPdata.Mid(40 , 8));

		strlistData[6] = m_tools._str16tostr10(m_tools._strDaoxu(strAllTPdata.Mid(40, 8)));//m_tools._str16tostr10(strAllTPdata.Mid(48 , 8));
		strlistData[7] = m_tools._str16tostr10(m_tools._strDaoxu(strAllTPdata.Mid(48, 8)));//m_tools._str16tostr10(strAllTPdata.Mid(56 , 8));
		
		strlistData[8] = m_tools._str16tostr10(m_tools._strDaoxu(strAllTPdata.Mid(56, 8)));//m_tools._str16tostr10(strAllTPdata.Mid(56 , 8));

		strlistData[9] = m_tools._str16tostr10(m_tools._strDaoxu(strAllTPdata.Mid(64, 4)));//m_tools._str16tostr10(strAllTPdata.Mid(64 , 8));
		//strlistData[11] = m_tools._str16tostr10(m_tools._strDaoxu(strAllTPdata.Mid(88, 8)));//m_tools._str16tostr10(strAllTPdata.Mid(72 , 8));

		strlistData[10] =m_tools._str16tostr10(m_tools._strDaoxu(strAllTPdata.Mid(68, 8)));//m_tools._str16tostr10(strAllTPdata.Mid(80 , 8));
		strlistData[11] =m_tools._str16tostr10(m_tools._strDaoxu(strAllTPdata.Mid(76, 8)));
		strlistData[12] =m_tools._str16tostr10(m_tools._strDaoxu(strAllTPdata.Mid(84, 8)));
		strlistData[13] =m_tools._str16tostr10(m_tools._strDaoxu(strAllTPdata.Mid(92, 8)));
		strlistData[14] =m_tools._str16tostr10(m_tools._strDaoxu(strAllTPdata.Mid(100, 8)));
		strlistData[15] =m_tools._str16tostr10(m_tools._strDaoxu(strAllTPdata.Mid(108, 8)));
		strlistData[16] =m_tools._str16tostr10(m_tools._strDaoxu(strAllTPdata.Mid(116, 8)));
		strlistData[17] =m_tools._str16tostr10(m_tools._strDaoxu(strAllTPdata.Mid(124, 8)));
		strlistData[18] =m_tools._str16tostr10(m_tools._strDaoxu(strAllTPdata.Mid(132, 8)));
		strlistData[19] =m_tools._str16tostr10(m_tools._strDaoxu(strAllTPdata.Mid(140, 8)));
		strlistData[20] =m_tools._str16tostr10(m_tools._strDaoxu(strAllTPdata.Mid(148, 8)));



		//		strlistData[1] = m_tools._str16tostr10(m_tools._strDaoxu(strAllTPdata.Mid(0, 8)));
		//strlistData[2] = m_tools._str16tostr10(m_tools._strDaoxu(strAllTPdata.Mid(8, 8)));//m_tools._strDaoxu(strMAC);//
		//strlistData[3] = m_tools._str16tostr10(m_tools._strDaoxu(strAllTPdata.Mid(16, 8)));//m_tools._strDaoxu(strAllTPdata.Mid(8 , 8));//
		//strlistData[4] = m_tools._str16tostr10(m_tools._strDaoxu(strAllTPdata.Mid(24, 8)));//m_tools._str16tostr10(strAllTPdata.Mid(16 , 8));
		//strlistData[5] = m_tools._str16tostr10(m_tools._strDaoxu(strAllTPdata.Mid(32, 8)));//m_tools._str16tostr10(strAllTPdata.Mid(24 , 8));

		//strlistData[6] = m_tools._str16tostr10(m_tools._strDaoxu(strAllTPdata.Mid(40, 8)));//m_tools._str16tostr10(strAllTPdata.Mid(32 , 8));
		//strlistData[7] = m_tools._str16tostr10(m_tools._strDaoxu(strAllTPdata.Mid(48, 8)));//m_tools._str16tostr10(strAllTPdata.Mid(40 , 8));

		//strlistData[8] = m_tools._str16tostr10(m_tools._strDaoxu(strAllTPdata.Mid(56, 8)));//m_tools._str16tostr10(strAllTPdata.Mid(48 , 8));
		//strlistData[9] = m_tools._str16tostr10(m_tools._strDaoxu(strAllTPdata.Mid(64, 8)));//m_tools._str16tostr10(strAllTPdata.Mid(56 , 8));

		//strlistData[10] = m_tools._str16tostr10(m_tools._strDaoxu(strAllTPdata.Mid(72, 4)));//m_tools._str16tostr10(strAllTPdata.Mid(64 , 8));
		//strlistData[11] = m_tools._str16tostr10(m_tools._strDaoxu(strAllTPdata.Mid(76, 8)));//m_tools._str16tostr10(strAllTPdata.Mid(72 , 8));

		//strlistData[12] =m_tools._str16tostr10(m_tools._strDaoxu(strAllTPdata.Mid(84, 8)));//m_tools._str16tostr10(strAllTPdata.Mid(80 , 8));
		//strlistData[13] =m_tools._str16tostr10(m_tools._strDaoxu(strAllTPdata.Mid(92, 8)));
		//strlistData[14] =m_tools._str16tostr10(m_tools._strDaoxu(strAllTPdata.Mid(100, 8)));
		//strlistData[15] =m_tools._str16tostr10(m_tools._strDaoxu(strAllTPdata.Mid(108, 8)));
		//strlistData[16] =m_tools._str16tostr10(m_tools._strDaoxu(strAllTPdata.Mid(116, 8)));
		...
		//strlistData[19] =m_tools._str16tostr10(m_tools._strDaoxu(strAllTPdata.Mid(140, 8)));
		//strlistData[20] =m_tools._str16tostr10(m_tools._strDaoxu(strAllTPdata.Mid(148, 8)));
		//strlistData[21] =m_tools._str16tostr10(m_tools._strDaoxu(strAllTPdata.Mid(156, 8)));




		CString strTopLine = _T("\n")+strlistData[0] + _T(",") + strlistData[1] + _T(",") + strlistData[2] + _T(",") + strlistData[3] + _T(",") + strlistData[4] 
			+ _T(",") + strlistData[5]+ _T(",") + strlistData[6]+ _T(",") + strlistData[7]+ _T(",") + strlistData[8] 
			+ _T(",") + strlistData[9]+ _T(",") + strlistData[10]+_T(",") +strlistData[11]+_T(",") +strlistData[12]+_T(",") +strlistData[13]+_T(",") +strlistData[14]+_T(",") +strlistData[15]+_T(",") +strlistData[16]+_T(",") +strlistData[17]+_T(",") +strlistData[18]+_T(",") +strlistData[19]+_T(",") +strlistData[20];
//		CString strTopLine = _T("\n")+strlistData[0] + _T(",") + strlistData[1] + _T(",") + strlistData[2] + _T(",") + strlistData[3] + _T(",") + strlistData[4] 
//			+ _T(",") + strlistData[5]+ _T(",") + strlistData[6]+ _T(",") + strlistData[7]+ _T(",") + strlistData[8] 
//			+ _T(",") + strlistData[9]+ _T(",") + strlistData[10]+_T(",") +strlistData[11]+_T(",") +strlistData[12]+_T(",") +strlistData[13]+_T(",") +strlistData[14]+_T(",") +strlistData[15]+_T(",") +strlistData[16]+_T(",") +strlistData[17]+_T(",") +strlistData[18]+_T(",") +strlistData[19]+_T(",") +strlistData[20];
		WideCharToMultiByte(CP_ACP,WC_COMPOSITECHECK,strTopLine,-1,szANSIString,sizeof(szANSIString),NULL,NULL);
...
INT8U CFView3762::RecvExtendStaLog(CString strName , INT8U *F8DataHead, int nChoose)
{
	TYPESTALOG=1;
	CMainFrame * pMain = (CMainFrame*)AfxGetApp()->GetMainWnd();
	CString str = _T("======================================================\r\n");
...
SENDCONTINUE:
		...
		case  0:
			nReadStart++;
			//Sleep(500);
			ack = gSimJzq.HostSendRcv376_2MsgAddrBuf(0x43 ,F8DataHead ,0xF0 , F32 , Sendbufdata , Sendbuflen , m_ptSendQGDW376_2 , m_ptRecvQGDW376_2);
//			ack = gSimJzq.HostSendRcv376_2Buf(0xF0 , F21 , Sendbufdata , Sendbuflen , m_ptSendQGDW376_2 , m_ptRecvQGDW376_2);
			break;
		case  1:
			nReadStart++;
			//Sleep(500);
			m_tools._str16tobuf16( pMain->m_strMAddress , AddrBuf , temp16 , true);
			ack = gSimJzq.HostSendRcv376_2F0F8MsgAddrBuf(0x43 , F8DataHead, AddrBuf ,0xF0 , F32 , Sendbufdata , Sendbuflen , m_ptSendQGDW376_2,ptRecvQGDW376_2 , m_ptRecvQGDW376_2);
//			ack = gSimJzq.HostSendRcv376_2F0F8(AddrBuf ,0xF0 , F21 , Sendbufdata , Sendbuflen , m_ptSendQGDW376_2,ptRecvQGDW376_2 , m_ptRecvQGDW376_2);
			break;
		}
		switch(ack)
...
/*				if(resendcnt > 5)
			...
			{
				if (m_ptRecvQGDW376_2.s_head.s_AFN == 0xF0 && m_ptRecvQGDW376_2.s_head.s_FN == F32)
				{
					CString strClear;
					m_editShow.GetWindowTextW(strClear);
					if(strClear.GetLength()>12000){
					...
					if(m_ptRecvQGDW376_2.s_RcvDataLen > 10)
					{
									CString strSn;
			             strSn.Format(_T(", sn %d"),nReadStart);
						SaveExtendStaLog(nReadStart-1, &m_ptRecvQGDW376_2.s_RcvDataBuf[0], m_ptRecvQGDW376_2.s_RcvDataLen);

						SetWindowText(strSn+_T(">>成功:") + m_tools._buf16tostr16(&m_ptRecvQGDW376_2.s_RcvDataBuf[0] ,m_ptRecvQGDW376_2.s_RcvDataLen , false , false) + _T("\r\n"));

						resendcnt = 0;
					}
					else
					{
						SetWindowText(_T(">>>>数据域数据过短") + m_tools._str16tostr10(m_tools._buf16tostr16(m_ptRecvQGDW376_2.s_RcvDataBuf , m_ptRecvQGDW376_2.s_RcvDataLen , false , false)) + _T("\r\n"));
					}
				}
		...
		case 0:
			...
			else
			{
				if (m_ptRecvQGDW376_2.s_head.s_AFN == 0xF0 && m_ptRecvQGDW376_2.s_head.s_FN == F158)
				{
					CString strClear;
					m_editShow.GetWindowTextW(strClear);
					if(strClear.GetLength()>20000){
						m_editShow.SetWindowTextW(_T(""));
					}
					if (m_ptRecvQGDW376_2.s_RcvDataLen > 10)
					{

						SaveExtendStaDevStatus(nReadStart-1, &m_ptRecvQGDW376_2.s_RcvDataBuf[0], m_ptRecvQGDW376_2.s_RcvDataLen);
					...
					{
						SetWindowText(_T(">>>>数据包过短") + m_tools._str16tostr10(m_tools._buf16tostr16(m_ptRecvQGDW376_2.s_RcvDataBuf, m_ptRecvQGDW376_2.s_RcvDataLen, false, true)) + _T("\r\n"));
					}
				}
...
DWORD WINAPI ThreadSendbufZuzhen (PVOID pParam) 
{
	...
	INT8U SendDatabuf[2000] , AddrBuf[6] , MainAddrBuf[7];
...
DWORD WINAPI ThreadSendbuf (PVOID pParam) 
{
	...
	for(POSITION pos = pMain->m_FenzuFnList.GetHeadPosition(); pos != NULL;)
	{
		...
		{
		case 0x15:
			pView->SendUpdataData(strDataName);
			break;
		case 0xFE:
...
//---------读运行日志 读STA版本号 特殊处理------------------------------------------------------------------------------------
					if(u8SendAFN == 0xF0 && u16SendFN == F21)  // AFN=F0, F21 读运行日志
					{
							...
							&& SendDatabuf[3] == 0x00 && SendDatabuf[4] == 0x00 && SendDatabuf[5] == 0x00)
						{
							...
							else if(SendDatalen==7){
								pView->RecvDebugData_7(strDataName, F8DataHead, SendDatabuf[6],0);
								break;
							}
						}
					}
					...
					//{
					//	TYPESTALOG=0;
					//	//sn=SendDatabuf[0]*16+SendDatabuf[1];
					//	if(SendDatabuf[0] == 0x00 && SendDatabuf[1] == 0x00 && SendDatabuf[2] == 0x00)
					//	{
					...
					//	}
					//}
					//if(u8SendAFN == 0xF0 && u16SendFN == F32)  // AFN=F0, F32 读STA故障信息
					//{
					//	TYPESTALOG=1;
					//	if(SendDatabuf[0] == 0x00 && SendDatabuf[1] == 0x00 && SendDatabuf[2] == 0x00)
					//	{
					//		pView->RecvExtendStaLog(strDataName, F8DataHead);
					//		break;
					//	}
...
DWORD WINAPI ThreadSendRepeat (PVOID pParam) 
{
	CMainFrame * pMain = (CMainFrame*)AfxGetApp()->GetMainWnd();
	CFView3762 *pView=(CFView3762*)pParam;
	sPartQGDW376_2CreatFrame ptSendQGDW376_2;
	...
	for(POSITION pos = pMain->m_FenzuFnList.GetHeadPosition(); pos != NULL;)
	{
		...
		{
		case 0x15:
			pView->SendUpdataData(strDataName ,1);
			break;
		case 0xFE:
...
//---------读运行日志  读STA版本号 特殊处理------------------------------------------------------------------------------------
					if(u8SendAFN == 0xF0 && u16SendFN == 0x0210)  // AFN=F0, F21 读运行日志
					{
							...
							&& SendDatabuf[3] == 0x00 && SendDatabuf[4] == 0x00 && SendDatabuf[5] == 0x00)
						{
							if(SendDatalen==6)
							{
								...
								pView->RecvDebugData_7(strDataName, F8DataHead, SendDatabuf[6],1);
								break;
							}
						}
					}
					...
					{
						TYPESTALOG=0;
						sn=SendDatabuf[0]+SendDatabuf[1]*16;
						if(SendDatabuf[0] == 0x00 && SendDatabuf[1] == 0x00 && SendDatabuf[2] == 0x00)
						{
					...
					if(u8SendAFN == 0xF0 && u16SendFN == F32)  // AFN=F0, F32 读STA版本号
					{
						TYPESTALOG=1;
						sn=SendDatabuf[0]+SendDatabuf[1]*16;
						if(SendDatabuf[0] == 0x00 && SendDatabuf[1] == 0x00 && SendDatabuf[2] == 0x00)
						{
							pView->RecvExtendStaLog(strDataName, F8DataHead, 1);
							break;
						}
					}
					if (u8SendAFN == 0xF0 && u16SendFN == F158)  // AFN=F0, 
					{
						if (SendDatabuf[0] == 0x00 && SendDatabuf[1] == 0x00 && SendDatabuf[2] == 0x00)
						{
					...
					case 0x00://成功
						...
						if ((temp16 + 2) == pView->m_ptRecvQGDW376_2.s_RcvDataLen)
						{
							...
							pView->DisplayData(strDataName ,strDatalist[2] , strDatalist[3] ,  ptRecvQGDW376_2.s_RcvDataBuf ,  ptRecvQGDW376_2.s_RcvDataLen);
								if(u8SendAFN == 0xF0 && u16SendFN == 0x1301)  // AFN=F0, F153 读STA版本号
								{
									pView->SaveExtendStaVersion(sn-1, &ptRecvQGDW376_2.s_RcvDataBuf[0], ptRecvQGDW376_2.s_RcvDataLen);
								}
								if(u8SendAFN == 0xF0 && u16SendFN == F32)  // AFN=F0, F32 读STA版本号
								{
									pView->SaveExtendStaLog(sn-1, &ptRecvQGDW376_2.s_RcvDataBuf[0],ptRecvQGDW376_2.s_RcvDataLen);
								}

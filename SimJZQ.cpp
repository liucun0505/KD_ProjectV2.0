...
INT8U CSimJzq::HostSendRcv376_2MsgAddrBuf(INT8U vSrcCn,INT8U * vSrcMsgAddr,INT8U vSrcAFN,INT16U vSrcFn,INT8U *vSrcBuf,INT16U vSrcLen,sPartQGDW376_2CreatFrame &sQGDW376_2HostFrame,TH_OUT sPartQGDW376_2DeCodeFrame &DstDecodeDataFrame,INT8U vSrcAckCmp)
{
	...
	if(ack == DACK_SUCESS)
	{
		for(INT8U mm = 0; mm < 1; mm ++)
		{
			...
			if(vSrcAckCmp == 0)
			{
				ack = Wait3762AckCmpSeq(10000 , 500 ,sQGDW376_2HostFrame,DstDecodeDataFrame);
			}
			else
			{
				ack = Wait3762AckCmpAFNFN(10000 , 500 ,sQGDW376_2HostFrame,DstDecodeDataFrame);
			}
			if(ack == DACK_SUCESS)
			{
				break;
...
void CSimJzq::GetCOMMStrToBuf(INT16U  getLen , INT32U nWaitFramTime , INT16U nWaiteByteTime ,INT8U * recvbuf,INT16U &recvlen)
{
	CMainFrame * pMain = (CMainFrame*)AfxGetApp()->GetMainWnd();
	INT16U len = 0;
	INT32U dlycnt=0;
...
INT8U  CSimJzq::WaitStrAck(INT32U nWaitFramTime , INT16U nWaiteByteTime,TH_OUT  INT8U * recvbuf){
	...
	INT16U recvbuflen=0;
	int nItem = 0;
	GetCOMMStrToBuf(2048, nWaitFramTime , nWaiteByteTime , recvbuf , recvbuflen);
	if(recvbuflen>0)
	{
		//ack = Cgw13762.Decode376_2Frame(recvbuf,recvbuflen,DstDecodeDataFrame);
		plog.saveLog_HexBuf(DATA_FILE,INPUT_DATA,recvbuf,recvbuflen);//打印接收帧数据

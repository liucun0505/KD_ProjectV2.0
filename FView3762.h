...
public://AFN=15 F001相关数据
	...
	void SetUpdataListData();//将升级文件保存到发送列表中
	CString GetDuanAndLen(int nduan,int nlen);
	CString strHEX(CString str,bool bDaoxu);
	INT8U SendUpdataData(CString strName , int nChoose = 0);//升级函数
	...
	INT8U RecvDebugData_7(CString strName, INT8U *F8DataHead ,INT8U data, int nChoose = 0);//读运行日志 数据长度为7时 临时加
	...
	INT8U RecvExtendStaLog(CString strName, INT8U *F8DataHead, int nChoose = 0);
	void SaveExtendStaLog(INT32U sn, INT8U *recvBuf, INT16U recvLen);
